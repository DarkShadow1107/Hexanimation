...
class Hexagon {
	constructor(kx, ky) {
		...
		for (let k = 0; k < 6; ++k) {
			...
			this.arcType[k] = ["s", null, "b", "s", null, "b"][(k - this.rot + 6) % 6]; // small or big
		} // for k
	} // Hexagon.constructor
	...
	size() {
		...
		this.middle = new Array(6).fill(0).map((p, k) => lerp(this.vertices[k], this.vertices[k + 1], 0.5));
	...
	constructor(line) {
        ...
        */
		...
		while (true) {
			...
			newa0 = (currArc.a0 + mPI + (currArc.ccw ? -1 : 1) * newAngle) % m2PI;
			newArc = new Arc(newCenter, newRadius, newa0, (currArc.a0 + mPI) % m2PI, !currArc.ccw);
		...
		for (let k = 0; k < nbLines; ++k) {
			...
			this.colors[k] = `hsl(${line.hue} ${line.sat}% ${80 - 50 * alpha}%)`;
		}
		line.segments.forEach((seg) => this.arcs.push(Object.assign(new Arc(), seg.arc)));
		/*
...
*/
		...
		this.lineLength = this.line.segments.reduce((s, v) => s + v.arc.len, 0);
	...
	draw(time) {
		...
		for (let kTrack = 0; kTrack < nbLines; ++kTrack) {
			...
			for (let k = kTail; k <= kHead; ++k) {
				if (k == kHead || k == kTail) this.arcs[k].addPathPartial(path, kTrack, this.headPos, tailPos);
...
function touchMove(event) {
	if (event.touches.length == 1) cursorPosition(event.touches[0].clientX, event.touches[0].clientY);
} // touchMove
